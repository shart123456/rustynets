...
async def advanced_fuzzing_demo():
    """Demo: Advanced fuzzing techniques"""
    print("\n" + "=" * 70)
    print("🚀 ADVANCED FUZZING DEMO")
    print("=" * 70)

    target = "https://example.com"

    # Load built-in wordlist
    base_wordlist = netool.load_wordlist(None)

    # Add common extensions
    extensions = ["php", "html", "asp", "aspx", "jsp", "txt", "bak", "old", "zip"]
    extended_wordlist = []

    print(f"\n[*] Generating extended wordlist...")
    for word in base_wordlist[:10]:  # Use first 10 words
        extended_wordlist.append(word)
        for ext in extensions:
            extended_wordlist.append(f"{word}.{ext}")

    print(f"[*] Extended wordlist size: {len(extended_wordlist)} paths")
    print(f"[*] Target: {target}")
    print(f"[*] Fuzzing with status filtering...")
    ...
    results = await netool.fuzz_directories(
        ...
        max_concurrent=50,
        timeout=10,
        status_filter=[200, 201, 301, 302, 403, 401]  # Common interesting codes
    )
    ...
    for status in sorted(by_status.keys()):
        ...
        for result in by_status[status][:5]:  # Show first 5
            print(f"  • {result['url']}")
...
async def main():
    ...
    print("  6. Advanced Fuzzing Techniques")
